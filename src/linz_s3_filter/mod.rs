...
impl Dataset {
    fn as_str(&self) -> &str {
        match self {
            Dataset::Elevation => "nz-elevation",
            Dataset::Imagery => "nz-imagery",
        }
    }
}

impl LinzBucket {
    pub async fn initialise_catalog(dataset: Dataset) -> Self {
...
mod tests {
    ...
    async fn test_get_hrefs() {
        ...
        let results = vec![MatchingItems {
            title: "Test Collection".to_string(),
            items,
        }];

        let hrefs = get_hrefs(results).await;
